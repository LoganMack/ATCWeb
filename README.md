...
- **Overall** (the default) — every class combined, ranked by `race_scores.scored_position`, which is what the points system is actually based on. Each row also shows which class that driver races in.
- **Per Class** — the original view: each class gets its own ranked table, re-derived from `curated_race_results` the same way `computeSeasonStandings()` does (see above).

Both views show a **Margin** column — the gap to the leader, formatted `xx.xxx` seconds, from `curated_race_results.interval_ten_thousandths` (`formatMargin()` in `src/lib/results.ts`).
