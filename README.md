...
**New Incident Report page.** `/results/[subsessionId]/incidents` replaces both the old external "Incident Report URL" link (v0.13) and the click-to-expand penalty panel on the results page (v0.14) with one real in-app page: a single table listing every incident logged for that round, reachable via a new "Incident Report" button next to the Overall/Per Class toggle. Columns: race, driver, incident #, lap, description, offense(s), involved cars, time/points/PP penalty, warning flag, appeal status/result, and — new — **Position Δ** and **Points Δ**, each driver's change versus a "before" snapshot of that race, measured across the round's *full* penalty recalculation rather than any single entry in isolation (so two penalties against the same driver in the same race show the same delta, reflecting where they actually ended up). All penalty add/edit/remove now lives exclusively on this page — `/results/[subsessionId]` itself is read-only for penalties, showing only the static tag next to each driver's name. The shared server-side logic (season PP recompute, form field parsing) moved into `src/lib/penaltyActions.ts` so both pages could use it without duplicating it, back when the results page still needed it.
