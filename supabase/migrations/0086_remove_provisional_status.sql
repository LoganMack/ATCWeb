-- Alpha Touring Challenge — remove the provisional/unofficial round-status gate
--
-- curated_rounds.status defaulted to 'provisional' on every row (NOT NULL,
-- DEFAULT 'provisional'), and recalculate_race_scores() silently no-op'd
-- (returned 0, no error, nothing written) for any round whose status wasn't
-- 'official'. This is exactly what happened to ATC18 Round 1's manually-
-- inserted data: it landed with the column's own default value and sat with
-- zero race_scores rows — no error, nothing on Standings/Career Stats/News
-- Recap, and no obvious reason why. Logan: "I do not think we need the
-- provisional results column/field/functionality at all anymore." This
-- removes the status GATE and the value-restricting CHECK constraint, and
-- flips the column's default to 'official' so any future insert that omits
-- the column (as apparently happened here) no longer lands in that same
-- dead state.
--
-- The column itself is left in place, not dropped — curated_rounds is one
-- of the tables this repo's own results-import pipeline shares with an
-- EXTERNAL process outside this codebase (see results.ts's header comment
-- and README's "results pipeline" notes); that process may still write an
-- explicit status value on insert, and dropping the column outright would
-- break any such write rather than just leaving it harmlessly unused. Every
-- reader in the app (src/lib/results.ts's RoundSummary, results.astro's
-- status badge, raceResultsImport.ts's CSV column) has been updated to stop
-- looking at it, so the column is now fully inert from the site's
-- perspective regardless of what value, if any, still lands in it.
--
-- recalculate_race_scores() below is CREATE OR REPLACE'd with the exact
-- same body as 0081_drop_unused_views_and_columns.sql, minus only the
-- 3-line status check (`if v_round.status <> 'official' then return 0; end
-- if;`) that used to sit right after the `delete from race_scores` line —
-- no other line changed.
alter table curated_rounds drop constraint if exists curated_rounds_status_check;
alter table curated_rounds alter column status set default 'official';
